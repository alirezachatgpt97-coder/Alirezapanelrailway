...
const ADMIN_USER=process.env.ADMIN_USER||"admin", ADMIN_PASSWORD=process.env.ADMIN_PASSWORD||"admin";
...
app.post("/api/login",(req,res)=>{const {username,password}=req.body||{};if(username!==ADMIN_USER||password!==ADMIN_PASSWORD)return res.status(401).json({error:"invalid credentials"});res.json({token:jwt.sign({sub:username},SECRET,{expiresIn:"12h"})})});
